import type { Invoice } from '../src/app/models/invoice.model';
...
export class InvoiceRenderer {
  private readonly cfg: { [K in keyof InvoiceCompany]: () => string };
  constructor(company: InvoiceCompany, private readonly size: '58mm' | '80mm' = '80mm') {
    this.cfg = {
  ...
  private paperSize(): '58mm' | '80mm' { return this.size; }
...
private formatDateTime(value: string | null | undefined): string {
    if (!value) return '';
    if (/^\d{4}-\d{2}-\d{2}$/.test(value)) return this.formatDate(value);
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return this.escape(value);
