...
function createMobileRouter({ prisma, loginLimiter, billing, provisionClient, queryIpam, reserveIp, releaseIp, whatsappStatus, sendWhatsapp, clientAction, externalClient, ticketProvider, liveNetwork, mikrotikOverview, mikrotikPing, linkTest, surveyConfig, invalidateSurveyConfig }) {
  ...
  router.use((error, _req, res, _next) => {
    // Los errores de WispHub y MikroTik traen `statusCode`; los propios, `status`.
    const known = Number(error.status || error.statusCode) || 0;
    const status = known >= 400 && known < 600 ? known : ['P2002', 'P2034', 'P2028'].includes(error.code) ? 409 : 500;
    res.status(status).json({ error: status === 500 ? 'No se pudo completar la consulta. Intenta de nuevo.' : known ? error.message : status === 409 ? 'Conflicto de operacion. Consulta el estado antes de reintentar.' : error.message, code: status === 500 ? 'MOBILE_API_ERROR' : known && typeof error.code === 'string' && !/^P\d/.test(error.code) ? error.code : 'REQUEST_FAILED' });
  });
  return router;
}
