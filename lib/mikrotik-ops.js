...
function buildIpamReport({ leases = [], arp = [], clients = [], queues = [], routerAddresses = [], pools = [], cidrs = [] }) {
    ...
    const classification = routerAddress ? 'router'
      : client ? 'client'
        : lease ? 'unknown_lease'
          : queue ? 'queue_only'
            : arpEntry ? 'arp_only'
              // Estar dentro de un pool DHCP no ocupa la IP: sin ARP, sin MAC (concesion),
              // sin cola, sin cliente y sin ser del router, esta libre. Al reservarla se
              // hace un ping ARP (probe_required) por si el DHCP la entrego hace poco.
              : 'available';
    return {
      ip,
      cidr,
      ...
      macAddresses: [...macs],
      // Para el ping ARP de la reserva: la interfaz del ARP o, si no hay, la del router en esa red.
      interface: (arpEntry || arpProbe)?.interface
        || routerAddresses.find((address) => ipInCidr(ip, address.address))?.interface || null,
      ...
      availabilityConfidence: arpProbe || poolName ? 'probe_required' : 'verified',
  ...
  for (const range of ranges) {
    // Primero las libres fuera de un pool DHCP; despues las que requieren sondeo.
    const candidates = rows.filter((row) => row.cidr === range.cidr && row.available)
      .sort((a, b) => Number(a.availabilityConfidence !== 'verified') - Number(b.availabilityConfidence !== 'verified'))
      .slice(0, 5);
  ...
  return {
    ...
    stats: {
      ...
      inPool: rows.filter((row) => row.poolName).length,
      poolReserved: 0,
