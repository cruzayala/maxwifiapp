...
function registerMobileWhatsapp(router, { prisma, wrap, permission, pagination, positiveId, searchWhere, statusProvider, sendWhatsapp }) {
  ...
  router.get('/whatsapp/bot/conversations', permission(['cobranza']), wrap(async (req, res) => {
    ...
    const logs = await prisma.whatsappLog.findMany({
      where: { OR: [{ messageType: 'incoming' }, { messageType: { startsWith: 'bot_' } }] },
      // Con busqueda se revisa mas historial para no perder conversaciones antiguas.
      orderBy: { createdAt: 'desc' }, take: req.query.q ? 5000 : Math.min(500, limit * 10),
    ...
    const term = String(req.query.q || '').trim().toLowerCase().slice(0, 120);
    const digits = term.replace(/\D/g, '');
    const matches = [...conversations.values()].filter((item) => !term ||
      String(item.clientName || '').toLowerCase().includes(term) || (digits.length >= 3 && String(item.phone || '').replace(/\D/g, '').includes(digits)));
    res.json({ items: matches.slice(0, limit), total: matches.length, source: 'sqlite', fetchedAt: new Date() });
  }));
