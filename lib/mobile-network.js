...
function registerMobileNetwork(router, { prisma, wrap, permission, pagination, positiveId, searchWhere, liveNetwork, mikrotikOverview, mikrotikPing, linkTest }) {
  ...
  // Prueba del enlace MikroTik -> equipo del cliente, la misma de la web. Es de solo
  // lectura, pero ocupa la cola de comandos del router: se atiende una a la vez.
  let linkTestBusy = false;
  if (linkTest) router.post('/clients/:id/link-test', permission(['tecnico']), wrap(async (req, res) => {
    const idServicio = positiveId(req.params.id);
    const seconds = boundedInt(req.body?.seconds, 8, 3, 20, 'Duracion de la prueba');
    if (linkTestBusy) fail('Hay otra prueba de enlace en curso. Espera unos segundos.', 429, 'LINK_TEST_BUSY');
    linkTestBusy = true;
    let result;
    try { result = await linkTest({ idServicio, seconds }); }
    finally { linkTestBusy = false; }
    if (result?.client?.idServicio !== idServicio || !result?.queue || !result?.traffic) {
      fail('MikroTik no confirmo la prueba.', 502, 'INVALID_MIKROTIK_RESPONSE');
    }
    await prisma.activity.create({ data: {
      action: 'mobile.mikrotik.link_test', entityType: 'client', entityId: String(idServicio), entityName: result.client.nombre || null,
      details: JSON.stringify({ actor: req.mobileUser.username, seconds: result.traffic.seconds, received: result.ping?.received ?? null }),
    } });
    res.json({ ...result, verified: true, source: 'mikrotik_live', fetchedAt: new Date() });
  }));
