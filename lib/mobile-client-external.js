...
function externalSnapshot(raw, idServicio) {
  ...
  const snapshot = {
    ...
    source: 'wisphub_live',
    profile: {
      displayName: profileName || text(detail.nombre || detail.servicio),
...
function validateChanges(body) {
  ...
  for (const key of keys) {
    ...
    if (key === 'displayName') clean[key] = boundedString(value, 'Nombre', 200, { required: true });
...
function registerMobileClientExternal(router, { prisma, wrap, permission, positiveId, provider }) {
  ...
  router.patch('/clients/:id/external', permission([]), wrap(async (req, res) => {
    ...
    try {
      ...
      const updatedRaw = await provider.update({ idServicio: id, section: input.section, changes: effective, before: beforeRaw });
