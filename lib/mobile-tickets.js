...
function registerMobileTickets(router, { prisma, wrap, permission, positiveId, provider }) {
  ...
  router.get('/tickets/:id/manage', permission(['tecnico']), wrap(async (req, res) => {
    ...
    catch (error) { throw Object.assign(new Error('No se pudo leer el ticket en WispHub. Revisa la conexion con WispHub e intentalo de nuevo.'), { status: error.status || 502, code: error.code || 'WISPHUB_TICKET_UNAVAILABLE' }); }
  }));
  ...
  async function mutate(req, res, creating) {
    ...
    try {
      ...
      catch (error) { throw Object.assign(new Error('WispHub no confirmo el cambio del ticket. Revisalo en WispHub antes de reintentar.'), { status: error.status || 502, code: error.code || 'WISPHUB_TICKET_WRITE_FAILED' }); }
