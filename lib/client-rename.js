...
const MISSING_IN_WISPHUB = 'Este cliente ya no existe en WispHub: fue borrado o cambio de numero de servicio. '
  + 'Revise en WispHub; ISP Max lo sigue mostrando solo porque quedo guardado de antes.';

/** Si WispHub dice que el servicio no existe, lo explica en vez de un error generico. */
async function readWisphubService(read, idServicio) {
  try {
    return await read(idServicio);
  } catch (error) {
    if (Number(error.statusCode || error.status) === 404) throw renameError(MISSING_IN_WISPHUB, 404, 'CLIENT_NOT_IN_WISPHUB');
    throw error;
  }
}
...
module.exports = { renameClientService, normalizeClientName, readWisphubService, MAX_LENGTH };
